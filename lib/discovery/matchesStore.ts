...
import { DISCOVERY_DIR, DISCOVERY_NEW_MATCHES_PATH } from "./paths";
...
export async function removeNewMatchesByJobId(jobId: string): Promise<{ removed: number }> {
  ...
  let raw = "";
  try {
    raw = await readFile(DISCOVERY_NEW_MATCHES_PATH, "utf-8");
  } catch {
    return { removed: 0 };
  }
...
export async function clearAllNewMatches(): Promise<void> {
  await mkdir(DISCOVERY_DIR, { recursive: true });
  await writeFile(DISCOVERY_NEW_MATCHES_PATH, "", "utf-8");
}
