...
NEGATION_SOFT_CHECK: Treat CONSTRAINTS as an exclusion only when they clearly reject something the job actually requires (quote both). If the job does not require what the user rejected, do not invent a veto or gap. Modal negation in CONSTRAINTS ("can't", "cannot", "don't") is not the same as rejecting a technical acronym the job requires (e.g. user "I can't speak English" must not be read as vetoing automotive CAN bus).
...
DASHBOARD_GEOGRAPHY (same policy shape as remote_zone, but for city/region vs dashboard preference): When DECISION_BRIEF.preferred_work_location is non-empty, compare it to DECISION_BRIEF.job_location and JOB_TEXT (office base, régió, site coverage). preferred_work_location may be a comma-separated list of cities and counties — treat ANY listed place, county-covered city, or obvious same-metro commuter town as aligned (do not use only the token before the first comma). Country words after a comma (e.g. "Hungary" in "Budapest, Hungary") are not extra cities. A Hungary-wide or Hungary-remote role aligns with Hungarian city/county lists. Company headquarters / HQ in another city (e.g. Prague) is not the work base when the role is hired into Hungary. Treat as material mismatch only when the required on-site/hybrid base is outside ALL of those places (e.g. listed Budapest/Pest vs Kecskemét-only site, or based in Prague covering Hungary). On material mismatch follow tactics.location: if default → vetoed=true, fit_score=0, veto_reason one EN sentence with quoted evidence from JOB_TEXT or DECISION_BRIEF.job_location plus a quoted preferred_work_location substring; metadata_fit_badge "Location Conflict". If strong_preference → vetoed=false, large negative constraint_delta (-15..-40), metadata_fit_badge "Location Conflict". If preferred_work_location is null/empty, skip this block.
...
PREF_LOC_JSON: ${preferredLocJson} — When non-empty: apply soft +2..+12 constraint_delta bonus only if DASHBOARD_GEOGRAPHY treats the job as aligned with any listed city/county (not first-comma-only). If DASHBOARD_GEOGRAPHY is a material mismatch, do not apply this positive bonus. Empty: neutral unless CONSTRAINTS ban a region.
