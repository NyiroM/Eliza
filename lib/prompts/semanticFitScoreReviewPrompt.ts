...
VETO (decide first): vetoed=true on any hard constraint clash (e.g. user bans a country/region and the job is based there, OR user explicitly excludes a required skill, OR user bans prospecting/hunting/new-client acquisition and the job is a new-business/hunter sales role), subject to CONSTRAINT_TACTICS. The server may already hard-veto prospecting clashes before this call — if HINTS say hard veto is expected for hunting roles, set vetoed=true. fit_score=0, score_components all 0, veto_reason one clear EN sentence naming the exact conflict; veto_reason MUST include a short quoted substring from JOB_TEXT or CONSTRAINTS (ASCII double quotes) proving the clash. breakdown ends Final Score: 0%. Else vetoed=false. Never say "rejected" in one_sentence_summary or narrative_summary unless vetoed=true.
