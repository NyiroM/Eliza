...
- **Per-listing AI reevaluate** — Discovery Hub match / not-a-match rows have a refresh control next to trash. It re-runs the Veto pipeline for that job only (`POST /api/discovery/reevaluate` with `job_id`) without clearing the rest of the lists.
...
- **Discovery fetch speed** — Indeed and Profession.hu reuse one Playwright browser across seed phrases. Profession listing-time detail visits default to **0**; Indeed hydrates **every SERP card’s JD** in that same session (`ELIZA_INDEED_DETAIL_VISITS`; set a number to cap, `0` to skip). Empty Indeed seeds fail fast (~6–10s card wait, no widening ladder). Profession tries two listing URLs then HTTP fallback (slug `/allas/…-id` links). Profession overall timeout default is **90s**. Live progress **Est. remaining** uses these paces.

### Fixed

- **Profession.hu detail enrich** — slug URLs (`/allas/title-city-1234567`) are fetched for JD text. The previous `/allas/`+digits-only check skipped them, so per-listing AI reevaluate dumped title-only Profession rows as non-matches. Verifier: **`npm run test:job-id-canonicalization`**.
- **AI reevaluate live progress** — finishing a reevaluate (full catalog or one listing) writes phase **`done`** (“Last run”) instead of clearing progress to idle, so the Live progress board stays on the hub. During Indeed JD hydrate the Fetch lane shows listing i/N, filled count, Chromium start, and remaining time.
...
- **Veto prose vs `vetoed=false`** — if the scorer writes “the application is vetoed…” but leaves `vetoed` false and a high `fit_score`, the parser now forces a hard veto and 0%. Discovery will not list those rows under New matches. Verifier: **`npm run test:hard-veto-policy`**.
...
- **Indeed title-only eval** — a new Playwright browser after listing fetch hits Indeed **Security Check**, so catalog/eval hydrate filled **0** JDs and every remaining Indeed row was dumped as title-only. Fetch now clicks **every** SERP card in the live search session (not only 4). Later catalog hydrate aborts on the first Security Check instead of walking 100 dead rows. Cap override: `ELIZA_INDEED_DETAIL_VISITS`. Catalog re-fetch cap: `ELIZA_INDEED_CATALOG_HYDRATE_MAX` (default 100, newest thin rows first).
