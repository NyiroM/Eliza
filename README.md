...
- **Suppressed listings** — jobs you remove with the trash control are stored as suppressed (**id + canonical URL**) in `storage/discovery/suppressed_ids.json` and are skipped on sync, re-evaluate, and queue processing until cleared (covers duplicate catalog lines or id drift for the same posting).
