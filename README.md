...
- **Constraint-aware** — saved preferences and hard vetoes (language, no-code / production-code, geography) surface in the dashboard and API; broader constraint clashes are decided by the semantic scorer with quoted evidence. **Target location** can trigger a deterministic location veto before semantic scoring.
...
- **Target location** — the dashboard **Target location** (saved preference, sent on sync as `preferred_location`) may be a **comma-separated list of cities and counties**. **Profession.hu** listing URLs and **Indeed** `l=` use the **first known city** (e.g. Budapest from `Budapest, Pest, Vác`). Country-only labels keep the national listing. **LinkedIn** guest search uses that same primary city (`Budapest, Hungary`), not the full comma string. Scoring/veto treats **any** listed place, county expansion, or Budapest-metro commuter town as aligned. A **Hungary-wide** (or Hungary-remote) role aligns with that list; a company **headquarters** or **office-country list** (e.g. Prague HQ, or offices in Latvia / Serbia / Hungary) is not the work base when the role is hired in Hungary or listed as Budapest.
- **Queue & progress** — jobs are evaluated with the same **pipeline** as a pasted posting; rows above your match threshold (and not vetoed) surface under **New matches**, with **company and title** on each row. The panel can **refresh match lists** when live stats change. After each sync, the server merges a **backlog** of pending catalog jobs (all providers in the configured tail window), not only the provider you just synced. **LinkedIn** detail enrich reads the numeric job id from slug URLs (`…/jobs/view/title-at-company-1234567890`) and falls back to Playwright; listings that stay title-only after that fetch are stored as **non-matches** (not 90%+ scores). **Indeed** hydrates each search-card JD in the **same Playwright session** as the listing scrape (default: every card; `ELIZA_INDEED_DETAIL_VISITS`). A later isolated browser hits Security Check, so leftover title-only rows are non-matches until you sync Indeed again. Pipeline crashes (Ollama timeout, etc.) retry with a cooldown; after the attempt cap the listing is written to **non-matches** with the error (not dropped). If the dashboard tab is closed, the **dev/start server** continues draining after cooldown (`ELIZA_DISCOVERY_SERVER_DRAIN=0` disables). A provider fetch that returns **0 listings without an HTTP error** is stored as `last_error` (likely empty scrape / block). **Ollama eval starts as soon as the first provider returns** (LinkedIn/Indeed while Profession is still scraping); set `ELIZA_DISCOVERY_EVAL_DURING_FETCH=0` to wait until all fetches finish. **Live progress** shows Fetch and Ollama as two tracks (what each is doing, remaining work, estimated finish time).
- **Re-evaluate** — **AI re-evaluate** (or `POST /api/discovery/reevaluate`) re-queues the catalog for scoring without a full re-fetch; useful after CV or constraint changes. A refresh icon next to trash on each match / not-a-match row re-scores **only that listing** (`job_id` on the same POST) and leaves the other rows in place.
...
**Optional server tuning** (see also comments in **`config/constants.ts`** and **`.env.example`**): `ELIZA_DISCOVERY_SYNC_EVAL_BATCH`, `ELIZA_DISCOVERY_QUEUE_DRAIN_BATCH`, `ELIZA_DISCOVERY_MAX_SEED_PHRASES` (default **10** phrases per provider per sync, clamp 1–10), `ELIZA_DISCOVERY_EVAL_DURING_FETCH` (set `0` to wait for all provider fetches before eval), `ELIZA_DISCOVERY_SERVER_DRAIN` (set `0` to disable in-process queue resume after failure cooldown), `ELIZA_DISCOVERY_PLAYWRIGHT` (set `0` for HTTP-only / RSS-style paths where supported), `ELIZA_DISCOVERY_HTTP_FETCH_TIMEOUT_MS` (default **45s** for LinkedIn guest / Indeed RSS / Profession list HTTP), plus Profession.hu / Indeed speed knobs `ELIZA_PROFESSION_FAST_NAV` (default **on**; set `0` for slower typing), `ELIZA_PROFESSION_LITE_SETTLE`, `ELIZA_PROFESSION_DETAIL_VISITS` (default **0** at search; JD at eval) / `ELIZA_INDEED_DETAIL_VISITS` (default **all SERP cards** in the fetch session; set `0` to skip), `ELIZA_PROFESSION_FETCH_TIMEOUT_MS` (default **90s**). Sync fetches **Indeed, LinkedIn, and Profession in parallel**, and reuses one Chromium per Playwright provider across the 10 seed phrases.
